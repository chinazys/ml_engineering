"""Run CIFAR-10 experiments with Weights & Biases tracking."""
...
def main() -> None:
    """Execute all configured experiments and log them to W&B."""
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--config",
        type=str,
        default="labs/lab05/configs/config.yaml",
        help="Path to Lab 05 config file",
    )
    ...
    logger.info(f"W&B project: {wandb_cfg['project']}")
    logger.info(f"W&B mode: {wandb_cfg.get('mode', 'online')}")
    ...
    for run_cfg in cfg["experiments"]:
        ...
        with wandb.init(
            ...
            run_dir = Path("artifacts/lab05") / run_name.replace(" ", "_").replace("/", "_")
            ...
            wandb.log_artifact(str(model_path), type="model")
            wandb.log_artifact(str(config_path), type="config")
            ...
            logger.info(
                f"Completed run {run_name} | test_acc={metrics['accuracy']:.4f} "
                f"test_f1={metrics['f1']:.4f}"
            )

    results_dir = Path("labs/lab05/results")
